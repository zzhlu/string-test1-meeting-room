...
，单价：${formattedPrice(receiptItem.price)}\
(元)，小计：${formattedPrice(receiptItem.payPrice)}(元)`;
  }).join('\n');

  return `***<没钱赚商店>收据***
${receiptItemsText}
----------------------
总计：${formattedPrice(receipt.totalPayPrice)}(元)
节省：${formattedPrice(receipt.totalSaved)}(元)
**********************`;
}
